...
public class BbsMagicAct {
	...
	public static final String TPL_BUY_LOG = "tpl.buy.log";
	public static final String TPL_GIVE_LOG = "tpl.give.log";
	public static final String TPL_MAGIC_SHOW = "tpl.magic.show";
	public static final String TPL_MAGIC_BUY = "tpl.magic.buy";
	public static final String TPL_MAGIC_NEW_BUY = "tpl.magic.new.buy";
	...
	public String magicshop(HttpServletRequest request, ModelMap model) {
		...
		if (!magicConfig.getMagicSwitch()) {
			return FrontUtils.getTplPath(request, site, TPLDIR_MAGIC, TPL_NO_OPEN);
		}
		...
		return FrontUtils.getTplPath(request, site, TPLDIR_MAGIC, TPL_MAGIC_SHOP);
	}
	...
	public String magicmybox(HttpServletRequest request, ModelMap model) {
		...
		List<BbsCommonMagic> shopMagics = magicMng.getList();
		model.addAttribute("memberMagics", magics);
		model.addAttribute("shopMagics", shopMagics);
	...
	public String magiclog(Integer pageNo, Integer tab, HttpServletRequest request,
		...
		Pagination page = magicLogMng.getPage(MagicConstants.MAGIC_OPERATOR_USE, user.getId(), SimplePage.cpn(pageNo),
				CookieUtils.getPageSize(request));
		model.addAttribute("tag_pagination", page);
	...
	public String buylog(Integer pageNo,Integer tab,HttpServletRequest request,HttpServletResponse response,
		...
		Pagination page = magicLogMng.getPage(MagicConstants.MAGIC_OPERATOR_BUY, user.getId(), SimplePage.cpn(pageNo),
				CookieUtils.getPageSize(request));
		model.addAttribute("tag_pagination", page);
		FrontUtils.frontPageData(request, model);
		FrontUtils.frontData(request, model, site);
	...
	public String givelog(Integer pageNo,Integer tab,HttpServletRequest request,HttpServletResponse response,
		...
		Pagination page = magicLogMng.getPage(MagicConstants.MAGIC_OPERATOR_GIVE, user.getId(),
	...
	public String getsellmagicpage(String mid, int operator, Integer pid, HttpServletRequest request,
		...
		if (magicConfig.getMagicSwitch()) {
			...
			if (magic.getCredit() == 1) {
				...
				sellMoneyCredit = magicMessage.getMessage("cmsUser.prestige");
			}
			// 折扣为0的情况返回重量选择丢弃操作，否则给出回收价格
			if (operator == 2) {
				model.addAttribute("magicMsg", magicMessage.getMessage("magic.weight", magic.getWeight()));
			} else if (operator == 0) {
				int sellMoney = (int) (magic.getPrice() * magicDiscount / 100);
				model.addAttribute("magicMsg",
						magicMessage.getMessage("magic.sell.discount", sellMoney, sellMoneyCredit));
			} else if (operator == 1) {
				String magicMsg = "";
				...
				if (permission == 1) {
					...
					// 不允许使用
					magicMsg = magicMessage.getMessage("magic.use.hasnobeusedpermission");
					permiss = false;
				}
	...
	public String getbuymagicpage(String mid, HttpServletRequest request, HttpServletResponse response,
		...
		model.addAttribute("userMagicPacketSize", user.getGroup().getMagicPacketSize() - user.getMagicPacketSize());
	...
	public void buymagic(String mid, int buynum, HttpServletRequest request, HttpServletResponse response) throws JSONException {
		...
		if (StringUtils.isNotBlank(mid)) {
			BbsCommonMagic magic = magicMng.findByIdentifier(mid);
			if (magic==null) {
				object.put("message", "\"未找到道具\"");
			}else{
				BbsMagicConfig magicConfig = magicConfigMng.findById(site.getId());
				String message = "";
				Integer paymoney = (int) (magic.getPrice() * buynum);
				try {
					String credit = "";
					if (magicConfig.getMagicSwitch()) {
						MagicMessage magicMessage = MagicMessage.create(request);
						if (buynum <= 0) {
							...
							if (magic.getCredit() == 1) {
								...
								if ((user.getPoint() - paymoney) < 0) {
									// 积分不足
									message = magicMessage.getMessage("magic.buy.point.noenough");
								} else {
									message = magicMessage.getMessage("magic.buy.success", paymoney, credit, buynum,
											magic.getName());
									// 需要增加用户道具数量，减少系统道具数量，减少道具包容量，减少用户积分
									userMng.updatePoint(user.getId(), -paymoney, null, mid, buynum, 3);
									magicLogMng.buyMagicLog(magic, user, buynum, MagicConstants.MAGIC_OPERATOR_BUY);
								}
							} else if (magic.getCredit() == 2) {
								credit = magicMessage.getMessage("cmsUser.prestige");
								if ((user.getPrestige() - paymoney) < 0) {
									// 威望不足
									message = magicMessage.getMessage("magic.buy.prestige.noenough");
								} else {
									message = magicMessage.getMessage("magic.buy.success", paymoney, credit, buynum,
											magic.getName());
									userMng.updatePoint(user.getId(), null, -paymoney, mid, buynum, 3);
									magicLogMng.buyMagicLog(magic, user, buynum, MagicConstants.MAGIC_OPERATOR_BUY);
								}
							}
						}
	...
	public void sellmagic(String mid, int operator, int num,
		...
		try {
			...
			if (operator == 0) {
				...
				if (magic.getCredit() == 1) {
					if (user_magic != null) {
						...
						if (user_magic.getNum() > num) {
							sellMoneyCredit = magicMessage.getMessage("cmsUser.point");
							userMng.updatePoint(user.getId(), totalMoney, null, mid, num, 0);
							object.put("magicMsg", magicMessage.getMessage("magic.sell.success", num, magic.getName(),
									totalMoney, sellMoneyCredit));
							magicLogMng.buyMagicLog(magic, user, num, MagicConstants.MAGIC_OPERATOR_SELL);
						} else {
							// 道具数量不够
							object.put("magicMsg", magicMessage.getMessage("magic.error.haslittlenum"));
						}
					} else {
						// 没有该道具
						object.put("magicMsg", magicMessage.getMessage("magic.error.hasnomagic"));
					}
				} else if (magic.getCredit() == 2) {
					if (user_magic != null) {
						if (user_magic.getNum() > num) {
							sellMoneyCredit = magicMessage.getMessage("cmsUser.prestige");
							userMng.updatePoint(user.getId(), null, totalMoney, mid, num, 0);
							magicLogMng.buyMagicLog(magic, user, num, MagicConstants.MAGIC_OPERATOR_SELL);
						} else {
							// 道具数量不够
							object.put("magicMsg", magicMessage.getMessage("magic.error.haslittlenum"));
				...
				if (user_magic != null) {
					if (user_magic.getNum() > num) {
						userMng.updatePoint(user.getId(), null, null, mid, num, 2);
						object.put("magicMsg", magicMessage.getMessage("magic.drop.success", num, magic.getName()));
						magicLogMng.buyMagicLog(magic, user, num, MagicConstants.MAGIC_OPERATOR_DROP);
					} else {
						object.put("magicMsg", magicMessage.getMessage("magic.error.haslittlenum"));
					}
				} else {
	...
	public String getmagicpage(Integer pid, Integer ppid, String mid,
		...
		if(StringUtils.isNotBlank(mid)){
			magic = magicMng.findByIdentifier(mid);
		}
		if(magic!=null){
			BbsMagicConfig magicConfig = magicConfigMng.findById(site.getId());
			Boolean permiss = true;
			...
			if (user_magic == null || user_magic.getNum() <= 0) {
				model.addAttribute("notFoundUserMagic", true);
			} else {
				model.addAttribute("notFoundUserMagic", false);
			}
			...
			if(magic!=null){
				...
				model.addAttribute("magicLogo", magic.getMagicLogo());
			}
			...
			return FrontUtils.getTplPath(request, site, TPLDIR_MAGIC, TPL_MAGIC_SHOW);
		}
			...
			if (magicConfig.getMagicSwitch()) {
						...
						if (StringUtils.isNotBlank(name)) {
							if (mid.equals(MagicConstants.MAGIC_SHOWIP)
								...
								if (userMng.usernameNotExist(name)) {
									magicMsg = magicMessage.getMessage("magic.finduser.error");
								} else {
									// 用户没有该道具，防止直接输入地址刷道具
									if (user_magic == null || user_magic.getNum() <= 0) {
										magicMsg = magicMessage.getMessage("magic.error.hasnomagic");
									} else {
										magicMsg = magicMessage.getMessage("magic." + mid + ".success");
										useMsg = topicMng.useMagic(request, site.getId(), pid, ppid, mid,
												user.getId(), RequestUtils.getIpAddr(request), color, userid);
										magicLogMng.buyMagicLog(magic, user, 1, MagicConstants.MAGIC_OPERATOR_USE);
									}
								}
							}
						} else {
							if (mid.equals(MagicConstants.MAGIC_SHOWIP)
									|| mid.equals(MagicConstants.MAGIC_CHECKONLINE)) {
								magicMsg = magicMessage.getMessage("magic.nousername.error");
							} else {
								if (user_magic == null || user_magic.getNum() <= 0) {
									magicMsg = magicMessage.getMessage("magic.error.hasnomagic");
								} else {
				...
				if (hasNoPermission(useMsg)) {
					...
					if (useMsg.contains(MagicConstants.MAGIC_NAMEPOST_SUCCESS)) {
						...
						username = useMsg.split(MagicConstants.MAGIC_NAMEPOST_SUCCESS)[1];
						object.put("magicMsg",
								magicMessage.getMessage(MagicConstants.MAGIC_NAMEPOST_SUCCESS, username));
					} else if (useMsg.contains(MagicConstants.MAGIC_CHECKONLINE_ONLINE)) {
						// 在线
						username = useMsg.split(MagicConstants.MAGIC_CHECKONLINE)[0];
						object.put("magicMsg",
								magicMessage.getMessage(MagicConstants.MAGIC_CHECKONLINE_ONLINE, username));
					} else if (useMsg.contains(MagicConstants.MAGIC_CHECKONLINE_OFFLINE)) {
						// 离线
						username = useMsg.split(MagicConstants.MAGIC_CHECKONLINE)[0];
						object.put("magicMsg",
								magicMessage.getMessage(MagicConstants.MAGIC_CHECKONLINE_OFFLINE, username));
					} else if (useMsg.contains(MagicConstants.MAGIC_SHOWIP_SUCCESS)) {
						// 窥视
						username = useMsg.split(MagicConstants.MAGIC_SHOWIP_SUCCESS)[0];
						ip = useMsg.split(MagicConstants.MAGIC_SHOWIP_SUCCESS)[1];
						object.put("magicMsg",
								magicMessage.getMessage(MagicConstants.MAGIC_SHOWIP_SUCCESS, username, ip));
					} else if (useMsg.contains(MagicConstants.MAGIC_MONEY_SUCCESS)) {
						// 金钱卡
						credit = useMsg.split(MagicConstants.MAGIC_MONEY_SUCCESS)[0];
						money = useMsg.split(MagicConstants.MAGIC_MONEY_SUCCESS)[1];
						object.put("magicMsg",
								magicMessage.getMessage(MagicConstants.MAGIC_MONEY_SUCCESS, credit, money));
					} else {
