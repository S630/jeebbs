...
public class UserPostAct {
	...
	public static final String MEMBER_REJECT_TOPIC = "tpl.memberRejectTopic";
		...
		if (user != null&&topicId!=null) {
			if(!user.getModerator()){
				...
				if(topic==null){
					...
					topic.setCheckStatus(BbsTopic.CHECKED);
		...
		if (user != null&&topicId!=null) {
			if(!user.getModerator()){
				...
				if(topic==null){
					...
					topic.setCheckStatus(BbsTopic.REJECT);
					topicMng.update(topic);
					status=1;
				}
			}
	...
	public void deleteTopic(Integer topicId,HttpServletRequest request,
		...
		if (user != null&&topicId!=null) {
			if(!user.getModerator()){
				...
				BbsTopic topic=topicMng.deleteById(topicId);
				if(topic==null){
					status=-1;
				}else{
					status=1;
