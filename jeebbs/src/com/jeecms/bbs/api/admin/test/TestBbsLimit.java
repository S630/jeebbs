...
public class TestBbsLimit {
	...
	public static void main(String[] args) {
		System.out.println(testLimitList());
	}
	
	private static String testLimitList(){
		String url = base+"/bbslimit/list";
	...
	private static String testLimitGet(){
		String url = base+"/bbslimit/get";
		StringBuffer paramBuff = new StringBuffer();
	...
	private static String testLimitSave(){
		String url = base+"/bbslimit/save";
		StringBuffer paramBuff = new StringBuffer();
		paramBuff.append("ip="+"123123");
	...
	private static String testLimitUdp(){
		String url = base+"/bbslimit/update";
		StringBuffer paramBuff = new StringBuffer();
		paramBuff.append("id="+1);//id 必填
		paramBuff.append("&ip="+"123123123");//名称 必填
		paramBuff.append("&userId="+2);//版位编号 必填 1 2 3 4 ----不可改
		paramBuff.append("&appId="+appId);
		String encryptSessionKey = "";
		try {
			encryptSessionKey = AES128Util.encrypt(sessionKey, aesKey, ivKey);
	...
	private static String testLimitDel(){
		String url = base+"/bbslimit/delete";
		StringBuffer paramBuff = new StringBuffer();
		paramBuff.append("ids="+"1,2,3");//id 必填
		paramBuff.append("&appId="+appId);
		String encryptSessionKey = "";
		try {
			encryptSessionKey = AES128Util.encrypt(sessionKey, aesKey, ivKey);
