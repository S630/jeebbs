...
public class TestBbsUserActiveLevelApiAct {
	...
	public static void main(String[] args) {
		System.out.println(testActiveLevelList());
	}
	
	private static String testActiveLevelList(){
		String url = base+"/activeLevel/list";
	...
	private static String testActiveLevelGet(){
		String url = base+"/activeLevel/get";
		StringBuffer paramBuff = new StringBuffer();
	...
	private static String testActiveLevelSave(){
		String url = base+"/activeLevel/save";
		StringBuffer paramBuff = new StringBuffer();
		paramBuff.append("levelName="+"qiufanceshi");
		paramBuff.append("&requiredHour="+2l);
		paramBuff.append("&levelImg="+"/jee/jee");
		paramBuff.append("&appId="+appId);
		String encryptSessionKey = "";
		try {
			encryptSessionKey = AES128Util.encrypt(sessionKey, aesKey, ivKey);
	...
	private static String testActiveLevelUpd(){
		String url = base+"/activeLevel/update";
		StringBuffer paramBuff = new StringBuffer();
		paramBuff.append("id="+4);
		paramBuff.append("&levelName="+"qiufanceshi22");
		paramBuff.append("&requiredHour="+2l);
		paramBuff.append("&levelImg="+"/jee/jee2");
		paramBuff.append("&appId="+appId);
		String encryptSessionKey = "";
		try {
			encryptSessionKey = AES128Util.encrypt(sessionKey, aesKey, ivKey);
